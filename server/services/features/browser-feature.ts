 ...
 *
 * When enabled (via config or `--enable=browser-control` in chat mode),
 * the tool becomes visible to the LLM AND the sidecar spawns. When
 * disabled, neither.
 *
 * One-shot `huko -- prompt` never spawns sidecars, so the tool may appear
 * in the LLM surface (if --enable=browser-control was passed) but browser
 * commands will fail with a clear "server not running" error.
 */
