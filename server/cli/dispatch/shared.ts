...
export function usage(exitCode: number = 3): never {
  ...
  out.write(
    [
      `${h("Usage:")} huko ${c("[flags]")} ${c("<prompt>")}      — talk to the agent (the default)`,
      ...
      d("  Flags come BEFORE the prompt. The first non-flag word starts the prompt;"),
      d("  everything after it is taken verbatim. Use `--` if your prompt starts with `-`."),
      "",
      ...
      `  ${c("huko setup")}`,
      "",
      d("  # daily use — no verb, prompt goes first"),
      ...
      `  ${c("huko -- --metric correctness")}             ${d("# `--` for prompts starting with -")}`,
      `  ${c("echo 'tricky & shell | chars' | huko")}     ${d("# pipe stdin — no quoting needed")}`,
