...
export function usage(exitCode: number = 3): never {
  ...
  out.write(
    [
      ...
      d("  Use `-` as the last token to read the prompt from stdin (`echo ... | huko -`)."),
      "",
      ...
      `  ${c("echo 'tricky & shell | chars' | huko -")}          ${d("# pipe stdin — `-` makes it explicit")}`,
      `  ${c("huko < prompt.txt")}                               ${d("# read prompt from a file (no `-` needed)")}`,
