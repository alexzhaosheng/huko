...
export function usage(exitCode: number = 3): never {
  ...
  out.write(
    [
      ...
      row("--lean", "Minimal prompt (~400 tok) + bash-only tools; ignores --role"),
      "",
      ...
      `  ${c("huko setup")}`,
      "",
      d("  # daily use (the `run` verb is optional when the first token starts with `-`)"),
      `  ${c("huko -- What is 2 + 2?")}                    ${d("# continues the active session")}`,
      `  ${c("huko --new -- new conversation")}            ${d("# starts fresh, switches active")}`,
      `  ${c("huko --show-tokens -- check the build")}     ${d("# show token breakdown after")}`,
