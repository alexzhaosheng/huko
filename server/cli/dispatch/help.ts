...
export function renderTopHelp(stream: ColorStream): string {
  ...
  return [
    ...
    "",
    H("Mode + budget (the two key per-call knobs):"),
    R("--lean", "Lean mode: minimal prompt + bash-only (default: full)"),
    R("--compact=<level>", "Context budget: concise | standard | extended | large | max"),
    "",
