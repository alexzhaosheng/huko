...
export function renderHelpText(stream: ColorStream): string {
  ...
  return [
    ...
    row("model show <ref>", "Show full record for a single model (incl. effective context window)"),
    row("model add <flags>", `Add a model; ${c("--project")} for project layer`),
    row("model update <ref> <flags>", "Patch fields on an existing model (e.g. --context-window)"),
    ...
    row("--project", "Write to <cwd>/.huko/providers.json instead"),
    "",
    h("Options for `model update`:"),
    row("--display-name=<text>", "Rename the model"),
    rowWithDefault("--think-level=<lvl>", "off | low | medium | high", "(replace only)"),
    rowWithDefault("--tool-call-mode=<mode>", "native | xml", "(replace only)"),
    row("--context-window=<n>", "Pin token budget; pass `auto` to clear pin"),
    row("--project", "Write to <cwd>/.huko/providers.json instead"),
    "",
    h("Options for `sessions list` / `provider list` / `model list` / `model show`:"),
    rowWithDefault("--format=<fmt>", "text | jsonl | json", "(default: text)"),
    "",
