 ...
 * cleanly into dist/cli.js, with no runtime filesystem lookup. Same
 * design rationale as `server/db/migrations.ts`: filesystem-relative
 * resolution from `import.meta.url` works in dev (tsx) but breaks
 * once the source moves into a bundle (`server/roles/coding.md`
 * isn't shipped, and the relative path differs).
 *
 * Each entry is `<name> → raw .md content (with optional ---
 * frontmatter fence at the top)`. `loadRole(name)` consults this map
 * as the third (lowest-precedence) layer after project / user
 * filesystem overrides.
 *
 * To add a built-in role: push another entry below. The repo's
 * `server/roles/<name>.md` file is no longer consulted at runtime,
 * but it's fine to keep it as the editable source — copy its content
 * here when you ship.
 */
