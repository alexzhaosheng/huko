...
server/cli/
  ...
  commands/
    run.ts                    `huko run` 实现
    sessions.ts               `huko sessions <verb>` 实现（list / delete）
  formatters/
...
resource 一个 dispatcher 函数（`dispatchSessions` 等），verb 在内部分发。
新增一个 verb 不动顶层路由，新增一个 resource 加一个 dispatch 函数 +
顶层一个 if 分支。
...
1. 跑 SqlitePersistence 的 migrations（幂等，与未来 daemon 共享 huko.db）
2. 验证有 `app_config.default_model_id`，没有就 exit 3 并提示去种子
3. 创建 chat session
   - 标题默认 = prompt 第一句去掉换行后的前 ~40 字符（截断时加 …）
   - 用 `--title=...` 显式覆盖
   - 完全空 prompt fallback 到 `cli YYYY-MM-DD HH:MM`
...
--title=<text>             显式 chat-session 标题（默认从 prompt 派生）
...
```bash
npm run huko -- run --memory "private question"
```

行为：

- session / tasks / entries 全在内存里，进程退出全消失
- providers / models / default_model_id **从 huko.db 读一次**，随后 SQLite
  连接关闭——本次 run 没有任何写盘
- `sessions list` 看不到这次的 session（因为没写进 DB）
- 适合 throw-away 提问 / CI 临时调用 / 不想污染历史记录的脚本

实现层在 `bootstrap.ts`，**只跟 Persistence 抽象打交道**：

```
if (ephemeral):
   sqlite = new SqlitePersistence()    # 构造函数自己 migrate（idempotent）
   memory = new MemoryPersistence()
   seedFromSource(memory, sqlite)      # copy providers/models/config
   sqlite.close()                       # ← 之后再无 DB 触碰
   persistence = memory
else:
   persistence = new SqlitePersistence()
```

bootstrap.ts **不知道 migration 这回事**——schema 管理是 SqlitePersistence
自己的实现细节，写在它的 constructor 里。这是 [架构原则](../architecture.md)
"Kernel 零基础设施假设"的具体落地。

种子时会重新 alloc 内存里的 id 并维护 id 映射，`models.providerId` 和
`default_model_id` 都被 remap 到新值。

---

## 当前实现：`huko sessions <verb>`

### `sessions list`

```bash
npm run huko -- sessions list
npm run huko -- sessions list --json
npm run huko -- sessions list --jsonl
```

只读 inspector，列出本地 SQLite 里的 chat sessions（**包括** 所有 `huko run`
跑过留下的会话）。按 `createdAt` 倒序。

输出：

| 模式 | stdout |
|---|---|
| `text`（默认） | 对齐表格：ID / TITLE / CREATED / UPDATED |
| `jsonl` | 每行一个 session JSON |
| `json` | 一份 JSON 数组 |

`createdAt` / `updatedAt` 在 JSON 模式下是 ISO 8601 字符串，在 text 模式下
是 `YYYY-MM-DD HH:MM`（locale-free，方便 grep / sort）。

### `sessions delete <id>`

```bash
npm run huko -- sessions delete 12
```

级联删除一个 session 以及它名下所有 tasks + entries（cascade 在 Persistence
层已实现）。**不交互式确认**——这是 single-user dev CLI，要保护用确认 prompt
反而难脚本化。要做 dry-run 自己 `sessions list` 看一眼再 delete。
...
- `sessions get <id>`        单条 session 详情（entry 计数等）
- `sessions show <id>`       渲染整段对话历史
- `tasks list [--session=N]`
- `models list`
- `providers list`

实现都是 ~30 行 + `persistence.{tasks,models,providers}.list()`；当前没用到所以先没加。
...
### 为什么默认 SqlitePersistence

为了与未来 daemon 共享 providers / models / 默认配置 + 让 `sessions list`
能看到所有历史。MemoryPersistence 的话每次 CLI 运行都得重新种子。`--memory`
flag 是给"不想留痕"的场景准备的——它复用 SQLite 里的模型配置，但本次 session
不写盘。
